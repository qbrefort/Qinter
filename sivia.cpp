...
Sivia::Sivia(repere& R,IntervalVector& iinside,double *rpos,int Qinter,int nbeacon,int &isinside,int &Sperhaps,double *err, double epsilon, double erroutlier) : R(R) {
...
//        xb = 25 -rand() % 50;
//        yb = 25 -rand() % 50;
//        Function dist(x,y,sqr(x-xb)+sqr(y-yb));
//        r=sqrt(pow((xr-xb),2)+pow((yr-yb),2));
//        NumConstraint *c1 = new NumConstraint(x,y,dist(x,y)<=(r+err[0])*(r+err[0]));
//        NumConstraint *c2 = new NumConstraint(x,y,dist(x,y)>=(r-err[0])*(r-err[0]));
//        NumConstraint *c3 = new NumConstraint(x,y,dist(x,y)>(r+err[0])*(r+err[0]));
//        NumConstraint *c4 = new NumConstraint(x,y,dist(x,y)<(r-err[0])*(r-err[0]));
//        cdistin.push_back(new CtcFwdBwd(*c1));
//        cdistin.push_back(new CtcFwdBwd(*c2));
//        cdistout.push_back(new CtcFwdBwd(*c3));
//        cdistout.push_back(new CtcFwdBwd(*c4));
//    }
//    CtcCompo outsidec(cdistout);
//    CtcUnion insidec(cdistin);
//    int maxq = 10; //nb of contractors
//    int ctcq = maxq - Qinter + 1; //nb for q-relaxed function of Ibex
//    CtcQInter inside(insidec,ctcq);
//    CtcQInter outside(outsidec,Qinter);
    ...
    double xr=rpos[0],yr=rpos[1];
    r1=sqrt(pow((xr-x1),2)+pow((yr-y1),2))*(1+erroutlier/100);
    ...
    r6=sqrt(pow((xr-x6),2)+pow((yr-y6),2));
    ...
    Function f6(x,y,sqr(x-x6)+sqr(y-y6));
    ...
    NumConstraint c61(x,y,f6(x,y)<=(r6+err[4])*(r6+err[4]));
    NumConstraint c62(x,y,f6(x,y)>=(r6-err[4])*(r6-err[4]));
    NumConstraint c63(x,y,f6(x,y)>(r6+err[4])*(r6+err[4]));
    NumConstraint c64(x,y,f6(x,y)<(r6-err[4])*(r6-err[4]));
    ...
    CtcFwdBwd out61(c61);CtcFwdBwd out62(c62);
    CtcFwdBwd in61(c63);CtcFwdBwd in62(c64);
...
//    Array<Ctc> Aout;
    ...
    int ctcq = maxq - Qinter + 1; //nb for q-relaxed function of Ibex
    if (nbeacon==2){
        Array<Ctc> Ain(inside1,inside2);
        Array<Ctc> Aout(outside1,outside2);
    ...
    if (nbeacon==3){
        Array<Ctc> Ain(inside1,inside2,inside3);
        Array<Ctc> Aout(outside1,outside2,outside3);
        CtcQInter inside(Ain,ctcq);
        CtcQInter outside(Aout,Qinter);
        IntervalVector box(2);
        box[0]=Interval(-25,25);
    ...
    if (nbeacon==4){
        ctcq = 4;
        Array<Ctc> Ain(inside1,inside2,inside3,inside4);
        Array<Ctc> Aout(outside1,outside2,outside3,outside4);
        CtcQInter inside(Ain,ctcq);
        CtcQInter outside(Aout,Qinter);
        IntervalVector box(2);
        box[0]=Interval(-25,25);
    ...
    if (nbeacon==6){
        Array<Ctc> Ain(inside1,inside2,inside3,inside4,inside5,inside6);
        Array<Ctc> Aout(outside1,outside2,outside3,outside4,outside5,outside6);
        CtcQInter inside(Ain,ctcq);
        CtcQInter outside(Aout,Qinter);
        IntervalVector box(2);
        box[0]=Interval(-25,25);
        ...
        R.DrawEllipse(x4,y4,re,QPen(Qt::black),QBrush(Qt::NoBrush));
        R.DrawEllipse(x5,y5,re,QPen(Qt::black),QBrush(Qt::NoBrush));
        R.DrawEllipse(x6,y6,re,QPen(Qt::black),QBrush(Qt::NoBrush));
        Ain.clear();
        Aout.clear();
